function LDtk_ext(file, tileSet,  level, change, layers, layers_begin){
	if (change == undefined) change = 0;
	if (level      == undefined) level      = 0;
	if (layers   == undefined) layers   = "all";
	if (layers_begin == undefined) layers_begin = 0;
	...
	///// Translate some given arguments
	if (layers == "all") {
		var len_layer = array_length(_rootStruct.levels[level].layerInstances)-1;
	}else{
		var len_layer = layers;
	}
	
	/// Go through the data ->
	for (var lay=len_layer; lay>layers_begin; lay--) { // For all layers
